...
const getNonce = async (req, res) => {
  try {
    ...
    res.json({ nonce: user.nonce, existingRole: user.role || null });
...
const verifySignature = async (req, res) => {
  try {
    ...
    let user = await User.findOne({ walletAddress: address });
    if (!user) {
      // Brand new wallet — create with the role from whichever portal they used
    ...
    // If user has no role yet — assign from portal
    if (!user.role || user.role === '') {
      user.role = role || 'freelancer';
    }

    // If user already has a role and tries wrong portal — block
...
const login = async (req, res) => {
  try {
    ...
    if (role && user.role !== role) {
      return res.status(403).json({
        error: `This account is registered as a ${user.role}. Please use the ${user.role} portal.`,
        correctRole: user.role
      });
    }
    const token = jwt.sign(
