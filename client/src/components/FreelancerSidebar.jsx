...
const FreelancerSidebar = () => {
  ...
  return (
    <aside className="w-64 flex flex-col py-8 px-4 min-h-screen relative"
      style={{ background: "linear-gradient(180deg, #021024 0%, #052659 100%)", borderRight: "1px solid rgba(84,131,179,0.2)" }}>

      <div className="absolute top-0 right-0 w-px h-full pointer-events-none"
        style={{ background: "linear-gradient(180deg, transparent, rgba(84,131,179,0.4), transparent)" }} />

      {/* Logo */}
      <div className="px-2 mb-8 cursor-pointer" onClick={() => navigate("/freelancer/dashboard")}>
        <div className="flex items-center gap-2.5 mb-1">
          <div className="w-7 h-7 rounded-lg flex items-center justify-center text-sm"
            style={{ background: "linear-gradient(135deg, #052659, #5483B3)", boxShadow: "0 0 15px rgba(84,131,179,0.4)" }}>
            ⛓
          </div>
          <span className="text-lg font-black" style={{ fontFamily: "Syne, sans-serif" }}>
            <span style={{ color: "#C1E8FF" }}>Free</span><span style={{ color: "rgba(255,255,255,0.6)" }}>Lance3</span>
          </span>
        </div>
        <div className="ml-9 text-xs tracking-widest uppercase"
      ...
      <div className="mx-2 mb-5 px-3 py-2.5 rounded-2xl"
        style={{ background: "rgba(84,131,179,0.1)", border: "1px solid rgba(84,131,179,0.2)" }}>
        <div className="text-xs mb-0.5" style={{ color: "rgba(193,232,255,0.5)", fontFamily: "Space Grotesk, sans-serif" }}>Signed in as</div>
        <div className="text-sm font-medium truncate" style={{ color: "#C1E8FF" }}>{user?.username || user?.email || "Freelancer"}</div>
      </div>
      ...
      <nav className="flex flex-col gap-1 flex-1">
        {items.map(item => (
          <button key={item.label} onClick={() => navigate(item.path)}
            ...
            style={isActive(item.path) ? {
              background: "linear-gradient(90deg, rgba(84,131,179,0.3), rgba(5,38,89,0.15))",
              border: "1px solid rgba(84,131,179,0.35)", color: "#C1E8FF"
            } : { border: "1px solid transparent", color: "rgba(125,160,202,0.5)" }}>
            {isActive(item.path) && (
              <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 rounded-full" style={{ background: "#5483B3" }} />
            )}
            <span className="text-base w-5 flex-shrink-0 text-center"
              style={{ color: isActive(item.path) ? "#7DA0CA" : "rgba(84,131,179,0.6)" }}>
              {item.icon}
            </span>
        ...
        <button onClick={() => navigate("/freelancer/notifications")}
          ...
          <span className="text-base w-5 text-center"
            style={{ color: isActive("/freelancer/notifications") ? "#7DA0CA" : "rgba(84,131,179,0.6)" }}>◎</span>
          <span className="text-sm font-medium flex-1" style={{ fontFamily: "Space Grotesk, sans-serif" }}>Notifications</span>
          {unread > 0 && (
            <span className="text-xs px-1.5 py-0.5 rounded-full font-bold"
              style={{ background: "#5483B3", color: "#C1E8FF", fontSize: "10px" }}>{unread}</span>
          )}
        </button>
      </nav>

      <div className="mt-4 pt-4" style={{ borderTop: "1px solid rgba(84,131,179,0.15)" }}>
        <button onClick={handleDisconnect}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 text-left"
          style={{ color: "rgba(193,232,255,0.4)" }}
          onMouseEnter={e => { e.currentTarget.style.color = "#ff8080"; e.currentTarget.style.background = "rgba(255,80,80,0.08)"; }}
          onMouseLeave={e => { e.currentTarget.style.color = "rgba(193,232,255,0.4)"; e.currentTarget.style.background = "transparent"; }}>
